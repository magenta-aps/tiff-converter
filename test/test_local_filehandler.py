...
def folder_structure_generator():
    ...
    """
    yield ('/base', ['folder1', 'folder2'], [])
    yield ('/base/folder1', ['sub1', 'sub2'], ['file1', 'file2'])
    yield ('/base/folder1/sub1', [], ['file3', 'file4'])
    yield ('/base/folder1/sub2', [], ['file5', 'file6'])
    yield ('/base/folder2', ['sub3', 'sub4'], ['file7', 'file8'])
    yield ('/base/folder2/sub3', [], [])
    yield ('/base/folder2/sub4', [], ['file9', 'file10'])
...
class TestLocalFileHandler(unittest.TestCase):
    ...
    def setUp(self, mock):
        ...
        self.handler = LocalFileHandler('/base')

    def test_should_return_folder1_file1(self):
        self.assertEqual('/base/folder1/file1', self.handler.get_next_file())

    def test_should_return_folder1_file2(self):
        self.handler.get_next_file()
        self.assertEqual('/base/folder1/file2', self.handler.get_next_file())

    def test_should_return_folder1_sub1_file3(self):
        self.handler.get_next_file()
        self.handler.get_next_file()
        self.assertEqual('/base/folder1/sub1/file3',
                         self.handler.get_next_file())

    def test_should_return_folder2_sub4_file10(self):
        self.next(9)
        self.assertEqual('/base/folder2/sub4/file10',
                         self.handler.get_next_file())
